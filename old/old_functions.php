...
function custom_rest_api_prepare_post( $post_response, $post, $context ) {
    $meta = get_post_meta($post['ID'], 'company', true); // get_post_meta( $post['ID'] );
    $post_response['meta_company'] = $meta;
 
    return $post_response;
}
add_filter( 'json_prepare_post', 'custom_rest_api_prepare_post', 10, 3 ); */

// include_once ('hpk.php');
// include_once ('functions_tambahan.php');
// include_once ('add_projects.php');

function wpdev_before_after($content) {
	global $post;

	$urbanhire_url = get_post_meta($post->ID, 'urbanhire_url', true);
	if(empty($urbanhire_url)){
		...
		$pattern ="/<img(.*?)class=\"(.*?)\"(.*?)>/i";
   		...
   		$content = preg_replace($pattern, $replacement, $custom_content);
   	}
...
function filter_pagetitle($title) {
	...
	// $title = str_replace($month, $this_month, $title);
	$title = substr_replace($title, $this_month, -4, -4);
...
function update_option_title(){
	...
	$this_month = bulan(date("m"));
	$wp_title = "Lowongan Kerja di Surabaya Terbaru ". $this_month . " " . date('Y');
	// $site_title = get_option( 'blogname' );

	if( $site_title !== $wp_title ) {
...
function page_navi($before = '', $after = '') {
	...
	for($i = $start_page; $i  <= $end_page; $i++) {
		if($i == $paged) {
			...
			// echo '<li class="page-item"><a class="page-link" ng-click="getPage('.$i.')" href="#'.get_pagenum_link($i).'">'.$i.'</a></li>';
			echo '<li class="page-item"><a class="page-link" ng-click="getPage('.$i.')" href="">'.$i.'</a></li>';
		}
	}
	//echo '<li class="">' . next_posts_link('Next ?') . '</li>';
	if ($end_page < $max_page) {
		//$last_page_text = ">>";
		// echo '<li class="page-item next"><a class="page-link" ng-click="getPage('.$i.')" href="#'.get_pagenum_link($max_page).'" title="Last">' . $max_page . ' </a></li>';
		echo '<li class="page-item next"><a class="page-link" ng-click="getPage('.$i.')" href="" title="Last">' . $max_page . ' </a></li>';
	}
	//echo '</ul></div>'.$after."";
}
...

{
	...
	register_sidebar(array(
		...
		'id' 			=> 'footer-1',  // Add only this line
